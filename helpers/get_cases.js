...
const getPlanDetails = async () => {
    try {
        const response = await axios.get(`https://api.qase.io/v1/plan/MAS/${testPlanId}`, { headers });
        ...
        const caseDetails = await Promise.all(caseIds.map(async (caseId) => {
            const caseResponse = await axios.get(`https://api.qase.io/v1/case/MAS/${caseId}`, { headers });
            return caseResponse.data;
        }));
        try {
            for (const caseResponse of caseDetails) {
                const { id, title, description } = caseResponse.result;
                const fileName = `${myArgs[0]}/features/${title.replace(/\s+/g, '_').toLowerCase()}.feature`;
